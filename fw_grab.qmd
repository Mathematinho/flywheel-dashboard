---
title: "FW Meta-data Daily Grab Log"
format:
  html:
    self-contained: true
execute:
...
df_files <- reticulate::py$files_df %>%
  ...
  mutate(time1 = session_label,
         time2 = info[[1]]$InstanceCreationTime,
         time3 = created,
         .after = acquisition_label)

df_files %>% saveRDS("grabbed.rds")

```

unique subjects: `r df_files %>% distinct(subject_label) %>% nrow()`
unique sessions: `r df_files %>% distinct(session_label) %>% nrow()`
unique acquisitions: `r df_files %>% distinct(acquisition_label) %>% nrow()`
total files: `r nrow(df_files)`
